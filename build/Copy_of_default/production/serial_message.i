...
# 2 "serial_message.c" 2
# 14 "serial_message.c"
static char outputMessage[10];

__asm("\tpsect eeprom_data,class=EEDATA,delta=2,space=3,noexec"); __asm("\tdb\t" "'0'" "," "'1'" "," "0" "," "0" "," "0" "," "0" "," "0" "," "0");
...
void setBaseMessage(char messageType, char msgAddress) {
 ...
 outputMessage[1] = msgAddress;
 outputMessage[2] = messageType;
 outputMessage[3] = chartox(combinedZones >>4);
    outputMessage[4] = chartox((combinedZones &15));



    char v = (fault_flags.value >> 8) &0xf;
    outputMessage[5] = chartox(v);

    v = (fault_flags.value >> 4) &0xf;
    outputMessage[6] = chartox(v);

    v = (fault_flags.value) &0xf;
    outputMessage[7] = chartox(v);
...
void process_set_zones_message(char * inmessage) {
    commsZones = (xtochar(inmessage[3]) << 4) + xtochar(inmessage[4]);
    combineZones();
    setBaseMessage('s', inmessage[1]);
...
void process_set_address_message(char * inmessage) {
    unsigned char new_address = inmessage[3];


    setBaseMessage('a', inmessage[1]);
    output_message(outputMessage);
...
process_message(char *inmessage) {
    ...
    if (inmessage[1] == '0' || inmessage[1] == address) {
