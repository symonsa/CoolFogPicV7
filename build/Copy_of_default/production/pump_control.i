...
# 206 "pump_control.c"
    ...
    TRISCbits.TRISC0 = 1; TRISCbits.TRISC1 = 1; TRISAbits.TRISA4 = 1; TRISAbits.TRISA5 = 1; TRISCbits.TRISC2 = 1; TRISB = 1;


    fault_flags.boostPumpBit = 0;
    fault_flags.lwl_fault = 0;
    fault_flags.lwp_fault = 0;
    fault_flags.lfp_fault = 0;
    fault_flags.po_fault = 0;
...
main(void) {
    char *message;
    char sendGetMessageBuffer[10] = {'0', '0', '0', '0', '0', '0', '0', '0', '0'};
    init();
    init_event_timer();
    ser_int();
    printf("waitingxxxx \n\r");
    ...
    while (1) {
        ...
        if (state == standbyState) {
            if (!timer_events[EventIdleTimeout].active) {
                ...
                enable_callback(EventIdleTimeout, (32767));
            }
