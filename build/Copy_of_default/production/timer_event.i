...
# 1 "C:\\Program Files (x86)\\Microchip\\xc8\\v2.05\\pic\\include\\c90\\limits.h" 1 3
# 7 "./pump_control.h" 2
# 110 "./pump_control.h"
extern char state;
...
extern volatile timer_event timer_events[EventTotalNumber];
...
typedef union {
    ...
    struct {
        unsigned lwl_fault : 1;
        unsigned lwp_fault : 1;
        unsigned lfp_fault : 1;
        unsigned po_fault : 1;
...
volatile timer_event timer_events[EventTotalNumber];
...
enable_callback(char callback, int time_delay) {
    if (!timer_events[callback].active) {
        ...
        timer_events[callback].active = 1;
    }
...
clear_callback(char callback) {
    timer_events[callback].active = 0;
}
...
init_event_timer(void) {
    ...
    timer_events[EventLowWaterLevel].callback = EventLowWaterLevelCallBack;;

    timer_events[EventLowWaterPressure].callback = EventLowWaterPressureCallBack;;
    timer_events[EventLowWaterPressureDebounce].callback = EventLowWaterPressureDebounceCallBack;;

    timer_events[EventLowFogPressure].callback = EventLowFogPressureCallBack;;
    timer_events[EventFaultStateLED].callback = EventFaultStateLEDCallBack;;
    enable_callback(EventFaultStateLED, 1);
    timer_events[EventIdleTimeout].callback = EventIdleTimeoutCallBack;;
    timer_events[EventDumpSolenoid].callback = EventDumpSolenoidCallBack;;
    timer_events[EventPODebounce].callback = EventPODebounceCallBack;;
    timer_events[EventRunSignalDebounce].callback = EventRunSignalDebounceCallBack;;
