...
monitor_pump_run(void) {
    ...
    if (GENERAL_FAULT_SIGNAL_ACTIVE) {
        // if PO goes hi  de-bounce for 0.5 sec if still hi then go to shutdown
        if (!timer_events[EventGeneralFaultDebounce].active) {
            timer_events[EventGeneralFaultDebounce].flag_bit = false;
            timer_events[EventGeneralFaultDebounce].next_state = shutdownState;
            enable_callback(EventGeneralFaultDebounce, PO_SIGNAL_DEBOUNCE_TIME);
        }
    }
